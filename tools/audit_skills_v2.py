...
def main():
    ...
    for i, s1 in enumerate(skills):
        ...
        for j, s2 in enumerate(skills[i+1:], start=i+1):
            ...
            ratio = difflib.SequenceMatcher(None, s1["body_raw"][:1500], s2["body_raw"][:1500]).ratio()
            if ratio > 0.85:
                group.append(s2)
                processed.add(j)
        if len(group) > 1:
    ...
    for g in near_duplicates:
        process_group(g, "Near Duplicate", "Medium (85%+ similarity)", "High text similarity.")
    ...
    with open(OUTPUT_MD, 'w', encoding='utf-8') as f:
        ...
        saved_bytes = sum(s["size"] for s in skills if s["action"] in ["REMOVE", "MERGE"])
